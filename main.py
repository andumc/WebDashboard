...
CONSOLE_FILTERS = [
    ...
    "UUID of player",
    "Disconnecting",
    "[Netty NIO Worker #3/INFO] [velocircon]:"
]
...
def send_rcon_command(server, command):
    ...
    if cmd.startswith("/"):
        cmd = cmd[1:]
    with MCRcon(server.get("rcon_host", "127.0.0.1"), server["rcon_password"], port=server["rcon_port"]) as mcr:
        return mcr.command(cmd)

@app.route("/dashboard.ico")
def favicon():
    return send_from_directory(r"C:\Web\Management", "dashboard.ico", mimetype="image/vnd.microsoft.icon")
...
def login():
    if request.method == "POST":
        ...
        if username in users and users[username] == password:
            ...
            TOKENS[token] = {"username": username, "last_active": time.time()}
...
def action(server_id):
    ...
    if server.get("remote"):
        try:
            res = requests.post(f"http://{server['agent_ip']}:{server['agent_port']}/action/{server_id}", json={"action": cmd}, timeout=5)
            if res.status_code == 200:
...
def send_command(server_id):
    ...
    if server.get("remote"):
        try:
            res = requests.post(f"http://{server['agent_ip']}:{server['agent_port']}/send/{server_id}", json={"cmd": cmd}, timeout=5)
            if res.status_code == 200:
                return jsonify(res.json())
            return jsonify(success=False, error=res.text), res.status_code
        except Exception as e:
...
def status():
    ...
    for sid, server in servers.items():
        ...
        info = {"status": "online" if online else "offline", "players": "N/A", "max_players": "N/A", "tps": "N/A", "cpu_percent": "N/A", "ram_mb": "N/A", "player_list": []}
    ...
    if "proxy" in servers:
        ...
        data["proxy"] = {"status": "online" if proxy_online else "offline", "players": str(total_players), "max_players": str(total_max) if total_max > 0 else "N/A", "tps": "N/A", "cpu_percent": "N/A", "ram_mb": "N/A", "player_list": []}
    return jsonify(data)
    ...
    if server.get("remote"):
        try:
            if request.method == "GET":
                res = requests.get(f"http://{server['agent_ip']}:{server['agent_port']}/properties/{server_id}", timeout=5)
            else:
                res = requests.post(f"http://{server['agent_ip']}:{server['agent_port']}/properties/{server_id}", json={"content": request.json.get("content", "")}, timeout=5)
...
def console(server_id):
    ...
    if server.get("remote"):
        try:
            res = requests.get(f"http://{server['agent_ip']}:{server['agent_port']}/console/{server_id}", timeout=5)
            return jsonify(res.json()), res.status_code
        except Exception as e:
...
def file_browser(root_name, subpath=""):
    ...
    items = []
    for name in sorted(os.listdir(current_path), key=lambda x: (not os.path.isdir(os.path.join(current_path, x)), x.lower())):
        full = os.path.join(current_path, name)
        rel = os.path.relpath(full, base).replace("\\", "/")
        items.append({"name": name, "is_dir": os.path.isdir(full), "rel": rel})
    parent = None
    ...
    if cur == ".":
        cur = ""
    return render_template("file_browser.html", items=items, parent=parent, cur=cur, root=root_name)
...
def edit_file(root_name, subpath):
    ...
    with open(full_path, "r", encoding="utf-8", errors="ignore") as f:
        content = f.read()
    return render_template("file_editor.html", content=content, path=subpath, root=root_name)
