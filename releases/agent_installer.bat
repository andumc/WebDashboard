...
:write_ui
echo.
echo [4/4] Writing agent_ui_launcher.py...

> agent_ui_launcher.py (
    echo import json
    echo import os
    ...
    echo.
    echo CONFIG_FILE = "agent_ui_config.json"
    echo DEFAULTS = {
    echo     "python_path": "py",
    echo     "script_path": "agent.py",
    ...
    echo         self.root = root
    echo         self.root.title("Agent.py Launcher UI")
    ...
    echo         tk.Button(btn_frame, text="Run agent.py", width=14, command=self.on_run).pack(side="left", padx=5)
    echo.
    ...
    echo         path = filedialog.askopenfilename(title="Select agent.py", filetypes=[("Python files", "*.py"), ("All files", "*.*")])
    echo         if path:
    ...
    echo             messagebox.showinfo("Started", "agent.py was started.")
...
echo Done.
echo You can now run: %PYTHON_CMD% agent_ui_launcher.py
echo.
pause
